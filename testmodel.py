...
data_count = 1
with torch.no_grad():
    for k in np.arange(0, total_num, data_chunk_size):
        end_id = min(total_num, k+data_chunk_size)
        ...
        data_loader = torch.utils.data.DataLoader(
            ...
            batch_size=1, shuffle=False, num_workers=args.workers * 2)
        ...
        for i, (data, label) in data_gen:
            ...
            if i % 10 == 0:
                print('video {}/{} done, {:.02f}%, average {:.5f} sec/video'.format(
                    i+data_count, total_num, (i+data_count)*100/total_num,
                    float(cnt_time) / (i+data_count)))
                # if i % 100 == 0:
